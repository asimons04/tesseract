...
This release of Tesseract adds an internal API to lookup domains against an export of the MBFC data to return the bias, credibility, reporting status, and questionable tags for a supplied news source (specified by their domain name).
...
**Note**: Users need to manually enable this feature.  It is located in App Settings -> Posts -> Media Bias Fact Check Badges


**API Endpoint**: `/tesseract/api/mbfc/lookup?domain={domain}`

Data is loaded from [https://github.com/drmikecrowe/mbfcext](https://github.com/drmikecrowe/mbfcext/blob/0a212eafa1d20940a99fbccf5f1d35d60b949316/docs/v5/data/combined.json) which seems to be generated by some kind of automated pull. I was unable to find any official dumps, so for now, I'll just pull that on each Tesseract build.  MBFC does not seem to have a public API I can hook into.  
...
- Consider adding an array of communities to the user settings where MBFC badges should be shown; they only really apply to news links but there's no way to differentate a news link for a regular link.
  - Will likely implement this in a future release. Just don't have time to plumb that in at present.
