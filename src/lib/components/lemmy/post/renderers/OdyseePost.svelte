<script lang="ts">
    ...
    export let postType: PostType           = 'odysee'
    ...
    let placeholderImage = '/img/odysee.webp'
...
{#if !compact}
    {#if clickToPlayClicked && inViewport && post.post.url}
        <OdyseePlayer url={post.post.url} title={post.post.alt_text ?? post.post.name} autoplay />
    {:else}
